...
function updateCounter(): void {
    /*
    var doneCouter: number = 0;
    
    for (var i = 0; i < todoliste.length; i++) {

        if (todoliste[i].todosChecked == true) {
            doneCouter = doneCouter++;
    ...
    */ //Funktioniert auch
    ...
    /*
    erledigtDOMElement.innerHTML = countercheck + " bereits erledigt";
    offenDOMElement.innerHTML = nochzuerledigen + " noch offen";
    */
    counterDOMElement.innerHTML = todoliste.length + " in total " + countercheck +" done, " + nochzuerledigen + " open";
    
    /*counterDOMElement.innerHTML = todoliste.length + " in total " + doneCouter +" done, " + (todoliste.length - doneCouter) + " open";*/
}
